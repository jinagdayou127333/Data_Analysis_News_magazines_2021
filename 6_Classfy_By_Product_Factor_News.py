...
nj = pd.read_excel('Temp/Final_ProductCluster_News.xls')
print(nj.info())
...
Data_=nj_product_pop.iloc[:,[1,3,4,5]] #年价结果更好
x_ = Data_.iloc[:, :3]
y_ = Data_.iloc[:, 3]
...
clf = Pipeline([("poly_features",PolynomialFeatures(degree=3)),("scaler",StandardScaler()),
...
print('训练准确度：\n', np.round(train_score,3))
print('测试准确度：\n', np.round(test_score,3))
print('使用的分类器是', clf, ';\n测试集上分类表现如下:')
