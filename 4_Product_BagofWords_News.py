...
nj = pd.read_excel('Temp/Final_Brief_News.xlsx')
nj['产品分类'] = nj['产品分类'].astype(str)
user_labels_list = []
...
for i, row in user_labels.iterrows():
    word = jieba.cut(re.sub(re_punctuation, "", row['产品分类']),cut_all = False) # test=0.2->0.9881
...
# df_cplb.to_csv('Temp/Product_Words_News.csv', encoding='gbk')